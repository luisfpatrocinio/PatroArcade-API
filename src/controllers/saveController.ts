...
export async function getPlayerSaveData(req: Request, res: Response) {
  try {
    ...
    if (isNaN(playerId) || isNaN(gameId)) {
      return res.status(400).json({ type: "playerSaveFailed", content: "IDs inválidos."})
    }
    ...
    return res.status(404).json({
      ...
      content: generateNewSaveDataShell(getRelevantPlayerId(req), Number(req.params.gameId)),
    });
  }
}
...
export async function savePlayerData(req: Request, res: Response) {
  ...
  if (isNaN(gameId)) {
      return res.status(400).json({ type: "playerSaveFailed", content: "Game ID inválido."})
  }
  ...
  try {
    ...
    if (!save) {
      console.log(`[savePlayerData] CRIANDO save: Player ${playerId}, Game ${gameId}`);
      save = new SaveData();
      ...
      if (!player || !game) {
        return res.status(404).json({ type: "playerSaveFailed", content: "Jogador ou Jogo não encontrado."})
      }
...
export async function updateRichPresence(req: Request, res: Response) {
  ...
  if (isNaN(gameId)) {
     return res.status(400).json({ type: "richPresenceFailed", content: "Game ID inválido."})
  }
   if (!richPresenceText) {
     return res.status(400).json({ type: "richPresenceFailed", content: "richPresenceText é obrigatório."})
  }
  ...
  try {
    ...
    if (!save) {
      console.log(`[updateRichPresence] CRIANDO save: Player ${playerId}, Game ${gameId}`);
      save = new SaveData();
      ...
      if (!player || !game) {
        return res.status(404).json({ type: "richPresenceFailed", content: "Jogador ou Jogo não encontrado."})
      }
    ...
    console.log(`[updateRichPresence] SUCESSO: Player ${playerId}, Game ${gameId}`);
